...
//create seed for add Pda
pub fn getStateSeeds(program_id: &Pubkey)  -> (Pubkey, u8) {
    return Pubkey::find_program_address(&[b"state"], program_id);
}



//create seed for profile Pda
pub fn getUserPostSeeds(initializer: &AccountInfo, program_id: &Pubkey) -> (Pubkey, u8) {
    return Pubkey::find_program_address(&[b"post", initializer.key.as_ref()], program_id);
}
...
pub fn process_instruction(
    ...
    match instruction {
        MovieInstruction::AddMovieReview {authority, title, rating, description } => {
            // Make a call to `add_move_review` function
            process_create_post(program_id, accounts, title, rating, description)
        }
    }
}
