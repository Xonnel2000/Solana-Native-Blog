...
export const NativeClient = async () => {
    //return true or false
    if (!!get(getNativeClient))
        return;
    //let useWalleter = get(walletStore)
    //let PubWalletKey= useWalleter?.publicKey
    // let newpublicKey = new PublicKey(PubWalletKey)
    //let cfg = config[get(selectedNetwork)];
    //get the nativeClientfrom lib nativeClient.ts 
    let NativeClient = await import('$lib/helpers/helperfunction/nativeClient');
    //we update the store with the detail return
    getNativeClient.update((_) => new NativeClient.default()); // establish our Solana connection & load our little library helpers
};